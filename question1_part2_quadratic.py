#% This code is for question1 part 2, the quadratic part————————dejin wang
...
# Convert training and validation datasets into PyTorch tensors and move them to GPU (if available)
X_train_tensor = torch.FloatTensor(X_train_poly).to(device)
y_train_tensor = torch.FloatTensor(y_train).view(-1, 1).to(device)  # Reshape y_train as a column vector

X_val_tensor = torch.FloatTensor(X_val_poly).to(device)
y_val_tensor = torch.FloatTensor(y_val).view(-1, 1).to(device)  # Reshape y_val as a column vector

# Define the Logistic Regression model
class LogisticQuadraticRegressionModel(nn.Module):
    def __init__(self, input_dim):
        super(LogisticQuadraticRegressionModel, self).__init__()
...
input_dim = X_train_poly.shape[1]  # Input feature dimension (polynomial features)
model = LogisticQuadraticRegressionModel(input_dim).to(device)

# Define the negative log-likelihood loss (using binary cross-entropy) and move it to GPU
criterion = nn.BCELoss().to(device)  # Binary cross-entropy loss is equivalent to negative log-likelihood
learning_rate = 0.01
...
with torch.no_grad():  # No need to compute gradients during validation
    ...
    y_val_pred_class = (y_val_pred >= 0.5).float()  # Convert probabilities to 0 or 1
...
# Visualize decision boundary for both training and validation datasets
def plot_decision_boundary(X, y, model, title):
    x_min, x_max = X[:, 0].min() - 1, X[:, 0].max() + 1
    y_min, y_max = X[:, 1].min() - 1, X[:, 1].max() + 1
    xx, yy = np.meshgrid(np.linspace(x_min, x_max, 100), np.linspace(y_min, y_max, 100))
    grid = np.c_[xx.ravel(), yy.ravel()]
    grid_tensor = torch.FloatTensor(poly.transform(grid)).to(device)  # Apply polynomial feature transformation
